...
class OP_CB_Admin {
    ...
    public static function handle_actions() {
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'delete_bridge' && isset($_GET['id'])) {
            $id = intval($_GET['id']);
    ...
    public static function handle_save_landing() {
        ...
        $user_id    = get_current_user_id();
        $landing_id = !empty($_POST['bridge_id']) ? intval($_POST['bridge_id']) : 0;
    ...
    public static function render_order_meta_box($post_or_order_object) {
        ...
        if ($bridge_name) {
            echo '<p style="margin:0 0 6px 0;"><strong>' . esc_html__('Bridge Campaign:', 'op-checkoutbridge') . '</strong> <br><span class="op-cb-badge op-cb-badge-primary" style="margin-top:2px;display:inline-block;">' . esc_html($bridge_name) . '</span></p>';
        }
        if ($event_id) {
