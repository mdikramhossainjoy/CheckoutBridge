...
class OP_CB_Admin {
    ...
    public static function register_admin_menu() {
        ...
        add_menu_page(
            __('CheckoutBridge', 'checkoutbridge'),
            __('CheckoutBridge', 'checkoutbridge'),
            'manage_options',
            $parent_slug,
        ...
        add_submenu_page(
            $parent_slug,
            __('Bridges Manager', 'checkoutbridge'),
            __('Bridges Manager', 'checkoutbridge'),
            'manage_options',
        ...
        add_submenu_page(
            $parent_slug,
            __('Documentation & Integration', 'checkoutbridge'),
            __('Documentation', 'checkoutbridge'),
            'manage_options',
            'checkoutbridge-docs',
    ...
    /**
     * Enqueue CSS & JS assets for admin (100% self-hosted, zero external CDN)
     */
    public static function enqueue_assets($hook) {
        if (strpos($hook, 'checkoutbridge') === false) {
            return;
        }

        $ver = defined('OP_CB_VERSION') ? OP_CB_VERSION : '1.0.0';

        // FontAwesome 6.5.1 — self-hosted
        wp_enqueue_style(
            'op-cb-fontawesome',
        ...
        wp_enqueue_style(
            'op-cb-font-space-grotesk',
            OP_CB_URL . 'assets/vendor/fonts/space-grotesk.css',
            array(),
            $ver
        );

        // jQuery Nice Select 1.1.0 — self-hosted
        wp_enqueue_style(
        ...
        $css_file = OP_CB_PATH . 'assets/css/admin.css';
        $js_file  = OP_CB_PATH . 'assets/js/admin.js';
        ...
        wp_enqueue_style(
            ...
            array('op-cb-font-space-grotesk', 'op-cb-fontawesome', 'op-cb-nice-select-css'),
            $css_ver
        );
        ...
        wp_localize_script('op-cb-admin-js', 'op_cb_vars', array(
            ...
            'i18n'     => array(
                'confirm_delete' => __('Are you sure you want to delete this bridge campaign config?', 'checkoutbridge'),
                'confirm_revoke' => __('Are you sure you want to revoke and regenerate this token? Any external landing pages using the old token will immediately lose access.', 'checkoutbridge'),
                'copied'         => __('Copied to clipboard!', 'checkoutbridge'),
                'selected'       => __('selected', 'checkoutbridge'),
                'created_msg'    => __('Bridge campaign created successfully.', 'checkoutbridge'),
                'updated_msg'    => __('Bridge campaign updated successfully.', 'checkoutbridge'),
                'deleted_msg'    => __('Bridge campaign deleted successfully.', 'checkoutbridge'),
                'revoked_msg'    => __('Bridge token key revoked & regenerated successfully.', 'checkoutbridge'),
            )
        ));
    }
    ...
    public static function handle_actions() {
        ...
        if (isset($_GET['action']) && ($_GET['action'] === 'delete_bridge' || $_GET['action'] === 'delete_landing') && isset($_GET['id'])) {
            ...
            if (isset($_REQUEST['_wpnonce'])) {
                ...
                if (wp_verify_nonce($nonce, 'op_cb_delete_bridge_' . $id) || wp_verify_nonce($nonce, 'op_cb_delete_landing_' . $id)) {
                    OP_CB_Bridge_Repository::delete($id);
                    set_transient('op_cb_flash_' . $user_id, array('message' => __('Bridge campaign deleted successfully.', 'checkoutbridge'), 'type' => 'info'), 60);
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'revoke_token' && isset($_GET['id'])) {
            ...
            if (isset($_REQUEST['_wpnonce'])) {
                $nonce = $_REQUEST['_wpnonce'];
                if (wp_verify_nonce($nonce, 'op_cb_revoke_token_' . $id)) {
                    OP_CB_Bridge_Repository::regenerate_token($id);
                    set_transient('op_cb_flash_' . $user_id, array('message' => __('Bridge token key revoked & regenerated successfully.', 'checkoutbridge'), 'type' => 'success'), 60);
                    wp_safe_redirect(admin_url('admin.php?page=checkoutbridge-bridges'));
                    exit;
                }
            }
    ...
    public static function handle_save_landing() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'checkoutbridge'));
        }

        if (isset($_POST['_wpnonce'])) {
            $nonce = $_POST['_wpnonce'];
            if (!wp_verify_nonce($nonce, 'op_cb_save_bridge_nonce') && !wp_verify_nonce($nonce, 'op_cb_save_landing_nonce')) {
                wp_die(__('Security check failed.', 'checkoutbridge'));
            }
        }

        $user_id    = get_current_user_id();
        $landing_id = !empty($_POST['landing_id']) ? intval($_POST['landing_id']) : 0;
        $name       = isset($_POST['name']) ? sanitize_text_field($_POST['name']) : '';
        $token      = !empty($_POST['token']) ? sanitize_text_field($_POST['token']) : OP_CB_Bridge_Repository::generate_token();
        $origins    = isset($_POST['allowed_origins']) ? sanitize_textarea_field($_POST['allowed_origins']) : '';
        $thank_you  = isset($_POST['thank_you_url']) ? esc_url_raw($_POST['thank_you_url']) : '';
        ...
        $assigned_products = isset($_POST['assigned_products']) && is_array($_POST['assigned_products'])
            ? array_map('intval', $_POST['assigned_products'])
            : array();
        ...
        if ($landing_id > 0) {
            ...
            set_transient('op_cb_flash_' . $user_id, array('message' => __('Bridge campaign updated successfully.', 'checkoutbridge'), 'type' => 'success'), 60);
        } else {
            OP_CB_Bridge_Repository::create($data);
            set_transient('op_cb_flash_' . $user_id, array('message' => __('Bridge campaign created successfully.', 'checkoutbridge'), 'type' => 'success'), 60);
        }
    ...
    public static function register_order_meta_boxes() {
        ...
        add_meta_box(
            ...
            __('CAPI Details', 'checkoutbridge'),
    ...
    public static function render_order_meta_box($post_or_order_object) {
        ...
        $landing_name = $order->get_meta('_op_cb_landing_name');
        ...
        if (empty($landing_name) && empty($event_id)) {
            echo '<p style="color:#64748b;font-size:12px;margin:0;">' . esc_html__('This order was not created via CheckoutBridge.', 'checkoutbridge') . '</p>';
            return;
        }

        echo '<div style="font-family:sans-serif;font-size:12px;line-height:1.6;color:#334155;">';
        if ($landing_name) {
            echo '<p style="margin:0 0 6px 0;"><strong>' . esc_html__('Bridge Campaign:', 'checkoutbridge') . '</strong> <br><span class="op-cb-badge op-cb-badge-primary" style="margin-top:2px;display:inline-block;">' . esc_html($landing_name) . '</span></p>';
        }
        if ($event_id) {
            echo '<p style="margin:0 0 6px 0;"><strong>' . esc_html__('Meta Event ID:', 'checkoutbridge') . '</strong> <br><code style="background:#f1f5f9;color:#3730a3;padding:2px 6px;border-radius:4px;font-size:11px;">' . esc_html($event_id) . '</code></p>';
        }
        if ($fbp) {
            echo '<p style="margin:0 0 6px 0;"><strong>' . esc_html__('Facebook _fbp:', 'checkoutbridge') . '</strong> <br><code style="background:#f1f5f9;color:#0f172a;padding:2px 6px;border-radius:4px;font-size:11px;">' . esc_html($fbp) . '</code></p>';
        }
        if ($fbc) {
            echo '<p style="margin:0 0 6px 0;"><strong>' . esc_html__('Facebook _fbc (fbclid):', 'checkoutbridge') . '</strong> <br><code style="background:#f1f5f9;color:#0f172a;padding:2px 6px;border-radius:4px;font-size:11px;">' . esc_html($fbc) . '</code></p>';
        }
        if ($client_ip) {
            echo '<p style="margin:0 0 4px 0;"><strong>' . esc_html__('Customer IP:', 'checkoutbridge') . '</strong> <code style="background:#f1f5f9;color:#475569;padding:2px 4px;border-radius:3px;font-size:11px;">' . esc_html($client_ip) . '</code></p>';
        }
        echo '</div>';
    }
}
