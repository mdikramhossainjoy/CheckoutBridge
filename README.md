...
* **Real-Time Dynamic Coupon Validator**: Dedicated `/validate-coupon` endpoint for real-time promo code validation and discount calculation before order submission.
...
* **Global Dual-Shield Anti-Bot Defense**: E.164 international phone number normalization and Client IP proxy velocity rate limiting to eliminate fake/spam COD orders.
...
| :--- | :---: | :--- |
| `/wp-json/checkoutbridge/v1/create-order` | `POST` | Ingests landing page customer details, items, and shipping choice to create a WooCommerce COD order. Returns a signed redirect token. |
...
## Quick Integration Example (PHP cURL)

```php
<?php
// 1. Validate Coupon Code (Optional)
$coupon_payload = json_encode([
    'landing_token' => 'op_cb_YOUR_CAMPAIGN_TOKEN',
    'coupon_code'   => 'FLASH50',
...
* **Prefix Scoping**: All classes, tables, options, transient keys, and DOM selectors are strictly scoped with `OP_CB_` / `op_cb_` to prevent namespace collision.
* **HPOS Ready**: Full native compatibility with WooCommerce High-Performance Order Storage (HPOS).
* **Sanitization & Escaping**: 100% strict input sanitization (`sanitize_text_field`, `sanitize_email`, `sanitize_textarea_field`) and context-aware output escaping (`esc_html`, `esc_attr`, `esc_url`).
